...
import { User, LogOut, Share2, UserPlus, TrendingUp, Award, Calendar, Lock, Eye, EyeOff, AlertCircle, BookOpen } from 'lucide-react';
import { 
  getCurrentUser, 
  setCurrentUser, 
  clearCurrentUser, 
...
export default function GestionProfils() {
  ...
  useEffect(() => {
    ...
    if (user) {
      ...
      loadJournalNotes(user.userId);
    }
  }, []);

  const loadJournalNotes = async (userId: string) => {
    try {
      const notes = await db.journalNotes.toArray();
      const userNotes = notes.filter(n => n.userId === userId).sort((a, b) => 
        new Date(b.date).getTime() - new Date(a.date).getTime()
      );
      setJournalNotes(userNotes);
    } catch (error) {
          ...
          <div className="flex items-center justify-between p-4 bg-gray-50 dark:bg-gray-700 rounded-xl">
            ...
            <button
              ...
              className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                user.sharePublic ? 'bg-indigo-600' : 'bg-gray-300 dark:bg-gray-600'
              }`}
            >
              <span className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                user.sharePublic ? 'translate-x-6' : 'translate-x-1'
              }`} />
            </button>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6">
          ...
          {journalNotes.length === 0 ? (
            <p className="text-gray-500 dark:text-gray-400 text-center py-8">
              Aucune note de journal pour le moment
            </p>
          ) : (
            <div className="space-y-4 max-h-[400px] overflow-y-auto">
              {journalNotes.map((note) => (
                <div key={note.id} className="border border-gray-200 dark:border-gray-700 rounded-xl p-4 bg-gray-50 dark:bg-gray-700/50">
                  ...
                  <p className="text-gray-700 dark:text-gray-300 whitespace-pre-wrap">{note.contenu}</p>
                </div>
              ))}
            </div>
          )}
