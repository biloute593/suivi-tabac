...
// Clé localStorage pour sauvegarder l'historique
const CHAT_HISTORY_KEY = 'chat_history_lydie';
...
export default function ChatIA({ onClose, cigaretteContext }: Props) {
  ...
  function addWelcomeMessage() {
    const welcomeMessage: Message = {
      ...
      content: cigaretteContext 
        ? `Salut Lydie ! 💚 Je vois que tu veux discuter de ta cigarette n°${cigaretteContext.numero} (${cigaretteContext.heure}). Comment tu te sens par rapport à celle-ci ?`
        : `Salut Lydie ! 💚 Je suis ton IA personnelle. Tu peux me parler de TOUT : ta journée, tes émotions, tes questions, ta réduction du tabac, ou juste discuter !\n\n💬 Quelques idées :\n• Parle-moi de ta journée\n• Pose-moi des questions sur n'importe quoi\n• Demande des conseils pour gérer une envie de fumer\n• Discute de ce qui te préoccupe\n\nJe suis là pour toi ! 🌟`,
      timestamp: new Date()
    };
  ...
  async function chargerContexteComplet() {
    try {
      ...
      const profilData = localStorage.getItem('suivi-tabac-profil');
      let profilInfo = { pseudo: 'Lydie' };
      if (profilData) {
        try {
          profilInfo = JSON.parse(profilData);
        } catch (e) {
  ...
  async function handleSend() {
    ...
    try {
      ...
      const contexte = {
        pseudo: fullContext?.profil?.pseudo || 'Lydie',
  ...
  function getFallbackResponse(userInput: string, cig: CigaretteType | null | undefined): string {
    ...
    if (lowerInput.includes('envie') || lowerInput.includes('craquer') || lowerInput.includes('fumer')) {
      return `💪 Je comprends Lydie, l'envie peut être forte. Voici quelques astuces :\n\n1. **Respire profondément** 3 fois\n2. **Bois un grand verre d'eau**\n3. **Attends 5 minutes** - l'envie passe souvent\n4. **Mâche un chewing-gum** ou mange une pomme\n5. **Change d'activité** - fais quelques pas\n\nTu as déjà fumé ${stats?.aujourdhui || 0} cigarettes aujourd'hui. Chaque cigarette évitée est une victoire ! 🎉`;
    }
    
    if (lowerInput.includes('conseil') || lowerInput.includes('aide') || lowerInput.includes('astuce')) {
      return `Voici mes conseils personnalisés pour toi Lydie 💚:\n\n📊 **Tes stats** : ${stats?.aujourdhui || 0} cigarettes aujourd'hui, moyenne de ${stats?.moyenne7j || 'N/A'}/jour\n\n🎯 **Objectifs** :\n• Identifie tes cigarettes "automatiques" (score < 10)\n• Essaie de retarder de 10 min chaque envie\n• Prends du Kudzu avant les moments difficiles\n\n💡 **Astuce du jour** : Note ce qui déclenche chaque cigarette - c'est la clé pour comprendre tes habitudes !`;
    }

    if (cig) {
      return `À propos de ta cigarette n°${cig.numero} à ${cig.heure} 🚬:\n\n• **Score** : ${cig.scoreCalcule}/30 ${cig.scoreCalcule <= 10 ? '(à supprimer en priorité !)' : cig.scoreCalcule <= 20 ? '(peut être réduite)' : '(importante pour toi)'}\n• **Besoin** : ${cig.besoin}/10\n• **Satisfaction** : ${cig.satisfaction}/10\n\n${cig.scoreCalcule <= 10 ? "Cette cigarette semble plutôt automatique. La prochaine fois, essaie d'attendre 5 minutes avant de la fumer - tu verras peut-être que tu peux t'en passer ! 💪" : "Cette cigarette t'apporte quelque chose. L'objectif n'est pas de te priver, mais de réduire celles qui ne t'apportent rien."}`;
    }

    if (lowerInput.includes('stress') || lowerInput.includes('anxieux') || lowerInput.includes('nerveux')) {
      return `Je comprends que tu te sentes stressée Lydie 🤗\n\nLe tabac donne l'impression de calmer, mais en réalité il augmente l'anxiété à long terme.\n\n**Alternatives anti-stress** :\n• 🧘 Respiration 4-7-8 (inspire 4s, retiens 7s, expire 8s)\n• 🚶 Marche rapide de 5 minutes\n• 🎵 Écoute ta musique préférée\n• 💧 Bois de l'eau fraîche\n• 🌿 Prends du Kudzu\n\nTu es plus forte que tu ne le penses ! 💚`;
    }

    return `Merci de partager ça avec moi Lydie 💚\n\nJe suis là pour t'accompagner dans ta réduction du tabac. Tu as fumé ${stats?.aujourdhui || 0} cigarettes aujourd'hui.\n\nN'hésite pas à me demander :\n• Des conseils pour gérer une envie\n• Une analyse de tes cigarettes\n• Des encouragements\n\nChaque pas compte, tu fais du bon travail ! 🌟`;
  }
