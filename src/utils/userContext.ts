...
export function getCurrentUser(): CurrentUser | null {
  ...
  // 2) Fallback: use default user from environment (helps production)
  const defaultUserId = (import.meta as any)?.env?.VITE_DEFAULT_USER_ID;
  const defaultPseudo = (import.meta as any)?.env?.VITE_DEFAULT_PSEUDO || 'Lydie';
  const defaultObjectif = Number((import.meta as any)?.env?.VITE_DEFAULT_OBJECTIF_GLOBAL || 12);
  if (defaultUserId) {
    const currentUser: CurrentUser = {
      userId: String(defaultUserId),
      pseudo: String(defaultPseudo),
      objectifGlobal: Number.isFinite(defaultObjectif) ? defaultObjectif : 12,
      sharePublic: false
    };
